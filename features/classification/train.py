...
# Per-user overall mean
user_mean   = ratings.groupby("userId")["rating"].mean()
above_mean  = genre_avg.sub(user_mean, axis=0) > 0           # genre avg > personal avg

# Signal 1: pure engagement — if you watch ≥10% of your movies in a genre you love it.
#   A low rating means the specific movie disappointed, not the genre.
#   41% Drama at 0.75★ → Drama fan who had bad luck with movie picks.
user_total  = genre_cnt.sum(axis=1).replace(0, 1)
genre_share = genre_cnt.div(user_total, axis=0)
engaged     = genre_share >= ENGAGEMENT_SHARE

# Signal 2: rates this genre above their personal mean with enough data
#   Catches genres they watch rarely but consistently enjoy (hidden preference)
quality     = above_mean & (genre_cnt >= MIN_RATINGS)
...
# ---- KNN ----
print("Training KNN...")
knn_base = KNeighborsClassifier(n_neighbors=15, weights="distance", metric="minkowski", p=2)
knn      = MultiOutputClassifier(knn_base, n_jobs=-1)
...
joblib.dump({
    ...
    "user_ids":     user_ids,
    "Y":            Y,           # actual labels — used for known users at prediction time
}, MODELS_DIR / "meta.pkl")

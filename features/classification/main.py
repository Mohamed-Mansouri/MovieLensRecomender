...
def _load():
    ...
    models = {"svm": svm, "knn": knn, "xgboost": xgb}
    ...
    def estimator_confidence(est, feat):
        if hasattr(est, "predict_proba"):
            probs = est.predict_proba(feat)[0]
            return float(probs[1]) if len(probs) > 1 else float(probs[0])

        if hasattr(est, "decision_function"):
            score = float(np.ravel(est.decision_function(feat))[0])
            return float(1.0 / (1.0 + np.exp(-score)))

        return float(est.predict(feat)[0])
    ...
    def predict(model_name, user_id):
        ...
        if user_id in Y.index:
            ...
            proba  = [estimator_confidence(est, feat) for est in model.estimators_]
            results = [
...
def classify(
    ...
    model:   str = Query("xgboost", description="svm | knn | xgboost"),
):
    valid_models = ["svm", "knn", "xgboost"]
